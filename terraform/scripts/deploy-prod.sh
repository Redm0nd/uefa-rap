...
# UEFA RAP Production Environment Deployment Script

set -e

echo "🚀 Deploying UEFA RAP Production Environment..."
...
# Variables
BACKEND_CONFIG="environments/prod-backend.conf"
...
terraform_plan() {
    ...
    terraform plan -var-file="$TFVARS_FILE" -out=tfplan-prod
...
terraform_apply() {
    ...
    echo -e "${RED}⚠️  WARNING: This is the PRODUCTION environment!${NC}"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo -e "${YELLOW}⚠️  Last chance to cancel. Proceeding with production deployment...${NC}"
        sleep 3
...
# Main execution
main() {
    echo -e "${RED}🚨 UEFA RAP PRODUCTION Environment Deployment${NC}"
    ...
    echo ""
    echo -e "${GREEN}🎉 Production environment deployment completed!${NC}"
    echo ""
    echo "Next steps:"
